...
public class UserServiceImpl implements UserInterface{
    ...
    User user = new User();
        ...
        String loginToken = String.valueOf(Math.random()).substring(2, 6);
  ...
  // SEND SMS MESSAGE WITH TWILIO
        ...
        Message message = Message
                .creator(new PhoneNumber(user.getPhonenumber()), // to
                        ...
                        "Dear Mr " + user.getFullname() + " your profile has been created please submit token " + loginToken + " to login ")
                .create();
    ...
    if(authorized == 0) throw new RuntimeException("Supply token details to log in");{
        String usertoken = user.getToken();
        if (token.equals(usertoken)){
            int authorizeUser = repository.authenticateUser(phonenumber);
    ...
    public int getTotalAmountOfRegisteredUsers(){
        int getTotal = repository.getTotalamountOfRegisteredUsers();
        return getTotal;
      }
        ...
        String role = "ROLE_BUSADMIN";
         ...
         user.setPhonenumber(userAdmin.getPhonenumber());
           ...
           Message message = Message
                   .creator(new PhoneNumber(user.getPhonenumber()), // to
                           new PhoneNumber("+18177797043"), // from
                           "DEAR MR " + user.getFullname() + " YOUR ADMIN PROFILE HAS BEEN SUCCESSFULLY CREATED")
                   .create();
